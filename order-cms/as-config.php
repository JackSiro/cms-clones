...
/*
	...
	More about this license: http://www.github.com/jacksiro/license.php
*/
...
*/
	...
	define('AS_MYSQL_DATABASE', 'kttcbus');
...
/*
	...
	5. Place all the AppSmata files on your server.
	6. Open the appropriate URL, and follow the instructions.

	More detailed installation instructions here: http://www.github.com/jacksiro/
*/
...
/*
	...
	MySQL database, or to include the AppSmata tables in an existing MySQL database.
*/
...
/*
	...
	If so, it is used instead of AS_MYSQL_TABLE_PREFIX as the prefix for tables containing
	information about user accounts (not including users' activity and points). This allows
	multiple APS sites to have shared logins and users, but separate posts and activity.

	define('AS_MYSQL_USERS_PREFIX', 'as_users_');
*/
...
/*
	...
	The key of each array element should be the standard part of the path, e.g. 'purchases',
...
/*
	...
	consult the online documentation on installing AppSmata with single sign-on.
...
/*
	Some settings to help optimize your AppSmata site's performance.

	If AS_HTML_COMPRESSION is true, HTML web pages will be output using Gzip compression, if
	...
	long lists of purchases, etc... As the start parameter gets higher, queries tend to get
	...
	when searching or finding related purchases. This saves time by ignoring words which are so
	...
	are not indexed efficiently. For example, this will enable browsing unreplied purchases per
	category. If your database becomes large, these queries could become costly.

	Set AS_OPTIMIZE_LOCAL_DB to true if your web server and MySQL are running on the same box.
	...
	Set AS_OPTIMIZE_DISTANT_DB to true if your web server and MySQL are far enough apart to
